...
plotByTotalDamageDollarsMean <- 
    ggplot(data = eventTypesByTotalDamageDollarsMean, mapping = aes(x = EVTYPE, y = log(TotalDamage.Dollars.Mean), fill = EVTYPE)) +
    geom_col() +
    coord_flip() + 
    scale_x_discrete(limits=rev(levels(eventTypesByTotalDamageDollarsMean$EVTYPE))) +
    geom_text(
        aes(x = EVTYPE, y = log(TotalDamage.Dollars.Mean) / 2, hjust=0, vjust=0.5,
            label = 
                paste(
