...
```{r plotHealthImpact,fig.width=10,echo=TRUE}
...
eventTypeColorScale <- scale_fill_manual(name = "EVTYPE", values = eventTypeColors)
...
plotByFatalitiesMean <- 
    ...
    eventTypeColorScale +
    labs(x = "Event Type", y = "Fatalities (mean)")
...
grid.arrange(plotByFatalitiesMean, plotByCasualtiesMean, ncol=2, nrow=1, name = "Mean Fatalities and Casualties by Event Type")
```
...
In order to show which types of events have the greatest economic consequences, we plot the top-20 event types ranked in order of largest total cost in dollars.

```{r plotEconomicImpact,fig.width=10,echo=TRUE}
# Define the number of event types to plot
numberToPlot <- 20

# Select the top 20 events, arranged by our various metrics
eventTypesByTotalCostMean <- 
    head(arrange(filteredSummarizedStormData, desc(TotalCost.Dollars.Mean)), n = numberToPlot)

# Sort them by their rank
eventTypesByTotalCostMean$EVTYPE <- factor(eventTypesByTotalCostMean$EVTYPE, levels = eventTypesByTotalCost$EVTYPE)

# Create a plot of events, ranked by mean total dollar cost
plotByTotalCostMean <- 
    ggplot(data = eventTypesByTotalCostMean, mapping = aes(x = EVTYPE, y = TotalCost.Dollars.Mean, fill = EVTYPE)) +
    geom_col() +
    coord_flip() + 
    scale_x_discrete(limits=rev(levels(eventTypesByTotalCostMean$EVTYPE))) +
    geom_text(
        aes(x = EVTYPE, y = TotalCost.Dollars.Mean / 2, hjust=0, vjust=0.5,
            label = 
                paste(
                    as.character(round(TotalCost.Dollars.Mean, digits=0)), 
                    paste("(N=", as.character(N), ")", sep="")
                )
            )
        ) + 
    theme(axis.text.y = element_blank(), axis.ticks.y = element_blank()) +
    labs(x = "Event Type", y = "Mean Total Cost [dollars]", title="Mean Total Cost by Event Type")
