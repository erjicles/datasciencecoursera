...
plotByTotalDamageDollarsMean <- 
    ggplot(data = eventTypesByTotalDamageDollarsMean, mapping = aes(x = EVTYPE, y = TotalDamage.Dollars.Mean, fill = EVTYPE)) +
    geom_col() +
    coord_flip() + 
    scale_x_discrete(limits=rev(levels(eventTypesByTotalDamageDollarsMean$EVTYPE))) +
    geom_text(
        aes(x = EVTYPE, y = TotalDamage.Dollars.Mean / 2, hjust=0, vjust=0.5,
            label = 
                paste(
                    as.character(round(TotalDamage.Dollars.Mean, digits=0)), 
                    paste("(N=", as.character(N), ")", sep="")
                )
            )
        ) + 
